...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        self.is_flying_hand = bool(is_flying_hand)
    ...
    def _build_robotwin_image_tensor(self, observation: Dict[str, Any]) -> torch.Tensor:
        ...
        if self.is_flying_hand:
            image = np.concatenate([
                _resize_rgb(obs_data["head_camera"]["rgb"], (320, 256)),
                _resize_rgb(obs_data["wrist_camera"]["rgb"], (320, 128)),
            ], axis=0)
        else:
    ...
    def _infer_action_chunk(self, observation: Dict[str, Any], instruction: str) -> np.ndarray:
        ...
        if self.is_flying_hand:
            state_vector = np.asarray(observation["flying_hand"]["state"], dtype=np.float32)
        else:
    ...
    def step(self, task_env, observation: Optional[Dict[str, Any]]) -> None:
        ...
        if self.is_flying_hand:
            ...
            planner.set_pose(task_env, sapien.Pose(action[:3].tolist(), [np.cos(action[3] / 2), 0, 0, np.sin(action[3] / 2)]))
            task_env.set_flying_hand_gripper(
                task_env.flying_hand_config["gripper"]["close_qpos" if action[4] >= 0.5 else "open_qpos"],
                is_grasp=action[4] >= 0.5,
            )
            planner.step(task_env, 15, save_freq=None)
