...
class ObjectViewer(BaseViewer):
    ...
    def load_actor(self, pose, inherit_config=None, inherit_type: Literal['force', 'advice'] = 'advice'):
        ...
        if self.config_path.exists():
            try:
                ...
                actor_config = self.normalize_config(actor_config)
    ...
    def normalize_config(self, actor_config: dict) -> dict:
        actor_config = deepcopy(actor_config)
        if "transform_matrix" not in actor_config and "trans_matrix" in actor_config:
            actor_config["transform_matrix"] = actor_config["trans_matrix"]
        if "contact_points_pose" not in actor_config and "contact_pose" in actor_config:
            actor_config["contact_points_pose"] = actor_config["contact_pose"]

        config = deepcopy(self.EMPTY_CONFIG)
        config.update(actor_config)

        orientation_point = config.get("orientation_point", [])
        if (
            isinstance(orientation_point, list)
            and len(orientation_point) == 4
            and all(isinstance(row, list) and len(row) == 4 for row in orientation_point)
        ):
            config["orientation_point"] = [orientation_point]
        return config
